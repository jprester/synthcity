  ...
  constructor(x, z) {
      ...
      for (let i = 0; i < 2; i++) {
        for (let j = 0; j < 2; j++) {
            ...
            if (window.game.environment.spotLights) {
              if (Math.random() < 0.1 && subtypeNoise > 0.8 && !topper)
                this.updateables.push(
                  new Spotlight(this.x + xOff, 160 * scale, this.z + zOff)
                );
            }
          }
          ...
          if (topper && adsType != null)
            this.updateables.push(
              new Topper(this.x + xOff, 190 * scale, this.z + zOff)
            );

          // smoke
          if (Math.random() < 0.05)
            this.updateables.push(
              new Smoke(this.x + xOff, 190 * scale, this.z + zOff)
            );
    ...
    if (
      ...
      var mesh = new Mesh(
        ...
        window.game.assets.getMaterial(mat)
      );
      mesh.position.set(
...
class Advert {
  constructor(x, y, z, geo, is_tower) {
    ...
    let mat = window.game.assets.getMaterial(
...
class Topper {
  constructor(x, y, z) {
    ...
    let geo = window.game.assets.getModel(
