...
class AssetManager {
  constructor(game, terminal = null) {
    ...
    this.buildingWindowsEmissiveIntensity = this.game.environment.windowLights
      ? 1.5
      : 0;
  ...
  load() {
    ...
    this.textures["env_night_windshield"].mapping =
      EquirectangularReflectionMapping;
    ...
    for (let i = 0; i < 10; i++) {
      ...
      this.textures["building_" + id + "_em"].anisotropy =
      ...
      this.textures["building_" + id + "_rough"].anisotropy =
        this.textureAnisotropy;
    }
    ...
    for (let i = 0; i < 5; i++) {
      let id = this.padNumber(i + 1);
      this.textures["ads_" + id] = this.textureLoader.load(
        this.path + "textures/ads_" + id + ".jpg",
      );
    }
    ...
    for (let i = 0; i < 3; i++) {
      let id = this.padNumber(i + 1);
      this.textures["smoke_" + id] = this.textureLoader.load(
        this.path + "textures/smoke_" + id + ".jpg",
      );
    }
    ...
    this.models["ground"] = new PlaneGeometry(
      this.game.cityBlockSize + this.game.roadWidth,
      this.game.cityBlockSize + this.game.roadWidth,
    );
